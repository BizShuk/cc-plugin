...
提供可跨 AI 編碼代理共用的自訂技能集與專屬代理定義，劃分為 `apple` (macOS 整合)、`explore` (抓取與摘要：MarkItDown, Scrapling, Firecrawl, Playwright)、`general` (通用技能：domain, project-explore, model-evaluator)、`god` (系統大一統理論：LLM 力學、領域探索、融合方法)、`review` (審查插件：consistency、business-improvement、folder-structure、naming-convention、doc-sync、dependency-hygiene、learning-document)、`tmp` (臨時測試)、`superpowers` (核心流程：TDD、除錯、協作模式 — vendored from obra/superpowers) 與 `gosdk` (Go 開發：code-quality、mvc、performance、network、dead-code、naming、zap→slog 遷移) 八個模組化插件目錄，並透過 `npx skills` CLI 安裝至 55+ 個支援的 AI Agent。
