...
var distillNoRetain bool

// DistillCmd distills observations into memory and fact stores.
var DistillCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		gbrainRoot := gosdkconfig.ExpandHome(viper.GetString("sources.gbrain_working.root"))
		...
		if len(facts) > 0 {
			tempDir := gosdkconfig.ExpandHome(viper.GetString("stores.mempalace.temp_dir"))
...
func init() {
	DistillCmd.Flags().BoolVar(&distillNoRetain, "no-retain", false, "Disable retention sweep after run")
}
