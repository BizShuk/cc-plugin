...
├── cmd/                      # Cobra CLI 子命令
│   ├── autop/                # 獨立 autop binary、PM2 task 與設定範本
│   │   ├── README.md         # autop 使用方式與功能契約
│   │   ├── CLAUDE.md         # autop 技術脈絡
│   │   ├── AGENTS.md         # symlink → CLAUDE.md
│   │   ├── README.todo       # autop 待辦事項
│   │   ├── docs/terminology.md # autop 術語單一來源
│   │   ├── plans/            # autop 設計計畫
│   │   ├── ecosystem.config.js # autop PM2 managed tasks
│   │   └── driver/           # 可重用 LLM CLI argv、stdin 與 credential process mapping
...
├── ecosystem.config.js      # 非 autop 的 PM2 常駐程序（Ollama、Agent Memory）
├── main.go                   # Go CLI 入口點
├── go.mod                    # Go 模組定義
...
- Key dependencies: `gosdk/config.ExpandHome`, `gocsv`, `yaml.v3`

## 關鍵決策 (Key Decisions)

- `Cobra + Viper` 組合：CLI 指令定義與設定管理標準模式，支援環境變數覆蓋
- `Viper 預設值單一來源`：`cc-plugin` 預設值集中於 `config/config.go`；`autop`
  的設定、命令與 process façade 技術脈絡集中於
  [`cmd/autop/CLAUDE.md`](cmd/autop/CLAUDE.md)
...
| ----------------- | -------------------------- | -------------------- |
| Autop CLI façade | [`cmd/autop/`](cmd/autop/) | package-specific entry points and decisions: [`cmd/autop/CLAUDE.md`](cmd/autop/CLAUDE.md) |
...
主要測試套件：`cmd/autop`, `cmd/export`, `cmd/memory`, `cmd/topology`,
`pkg/topology`, `scripts/pluginmeta`
...
- Configuration: 設定路徑統一使用 `~` 前綴並展開；cc-plugin 預設值寫在
  `config/config.go`，autop 的設定規則見 [`cmd/autop/CLAUDE.md`](cmd/autop/CLAUDE.md)
