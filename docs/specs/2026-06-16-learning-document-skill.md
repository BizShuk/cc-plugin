# 建立教程技能 (Tutorial Skill) 實作計畫 (Tutorial Skill Implementation Plan)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

`Goal:` 在 `plugins/review/skills/` 底下建立 `tutorial` 技能，用以指導 AI 代理為當前專案生成逐步的教程，並分類存放於 `./docs/` 與 `./docs/tutorials/` 目錄中，且對關鍵術語進行特別高亮與解釋。
...
### Task 1: 建立教程技能 (Tutorial Skill)

`Files:`
- Create: `plugins/review/skills/tutorial/SKILL.md`

- [ ] **Step 1: 建立技能檔案**

建立 `../plugins/review/skills/tutorial/SKILL.md`，內容須包含 `Use when...` 起頭的 YAML frontmatter 觸發條件描述，且以 Traditional Chinese 撰寫技能指引。
...
name: tutorial
description: >
  Use when the user requests step-by-step guides, onboarding documents, or learning materials to understand the project workspace. Triggers on "create learning document", "generate tutorials", "教程", "學習指南".
version: "1.0.0"
...
## 概述 (Overview)
本技能用以引導 AI 代理針對當前工作區 (Workspace) 產生結構化的逐步教程，協助讀者快速理解專案的領域知識與架構。
...
3. `逐步引導 (Step-by-Step Guide):`
   - 教學或教程必須使用循序漸進的步驟格式（例如 `Step 1`, `Step 2`）。
...
修改 `../plugins/review/.claude-plugin/plugin.json`，將新技能的相對路徑 `./skills/tutorial` 加入 `skills` 陣列中。
...
根據 CLAUDE.md 中「更新插件時也必須同步更新對應的 README.md」的規則，修改 `../plugins/review/README.md`，在技能列表中加入 `tutorial` 的描述。

修改處：
在目錄結構與技能對照表中加入 `tutorial` 相關內容。
...
Run: `npx skills add .`
Expected: 輸出中應包含 `tutorial` 技能的安裝與連結成功訊息。
