...
| P5  | **No mega-packages either.** A single `types`, `models`, `api`, `interfaces`, or `dto` package that holds everything has the same problems as `util`. Group types with the code that operates on them, or split into focused subpackages. | `package types` (300 structs) | `package order`, `package payment`, `package inventory`          |
