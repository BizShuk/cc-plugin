...
`plugins/` 只收納八個本地 plugin；外部 plugin 由根目錄
...
每個本地 plugin 都有 `.claude-plugin/plugin.json` 與 `README.md`，八個都登錄於
...
根 marketplace 目前引用四個 GitHub 來源：`gosdk`、`inf`、`ip-incubation`、`pm2`。
