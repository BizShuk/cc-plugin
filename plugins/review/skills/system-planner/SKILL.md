...
當您面臨以下情境時，應使用本技能：
- 規劃新功能 (feature) 的架構位置、邊界定義、資料流與漸進落地步驟，輸出 `plans/architecture-<feature_name>.md` 時。
...
規劃新功能架構，確保整體保持清晰 (clear) 且高度可擴充 (highly scalable)。規劃結果寫入 `plans/architecture-<feature_name>.md` 與 `README.todo`。
...
7. `寫入計畫`：輸出 `plans/architecture-<feature_name>.md` 並在 `README.todo` 追加待辦行。
...
### Mode 1 Output Structure (plans/architecture-*.md)
...
### Mode 2 Output Example (Conscise Review Report)

```text
Codebase quality review
- [conflict] consistency  cmd/auth.go:42 ↔ config/jwt.go:12 — secret key defaults mismatched
- [unpinned] dependency   go.mod:14 — github.com/x/lib version range not pinned
- [loose]    structure    cmd/utils.go → move into pkg/utils/
- [concise]  code quality model/user.go:88 — remove unused variable 'lastActiveIP'
```
