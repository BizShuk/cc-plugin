...
| 錯誤 | 修正 |
| ---- | ---- |
| 把文件裡的斷言原樣搬進測試 | 先實跑；斷言預設為錯，本次實測 11 條有 2 條是紅的 |
| 用`遞移閉包`驗證`直接依賴` | `-deps` 會帶進上游的依賴；要斷言直接 import 就查 `Imports` |
| 用 `grep` 文字比對代替語意檢查 | 註解與測試字串是合法命中；斷言 import 而非字串 |
| guard test 一次都沒紅過就交付 | 注入違規證明它會 FAIL 且指名違規者，再還原 |
| subtest 名稱含 `/` | `-run` 會靜默匹配不到並印 `no tests to run`，看起來像成功 |
| 用行號做多段刪除 | 前面的刪除會位移後面所有行號；改用 anchor 文字 |
| 沒查目的地就刪 | 先實測目的地已有該內容，否則是遺失不是搬移 |
| 只檢查改動處 | 刪完重讀全文；被刪表格會留下 `(M2)`、`見上表` 之類懸空引用 |
| 稽核工具寫檔弄髒 repo | 驗證必須唯讀；建置導向暫存目錄並清理，最後用 `git status --short` 核對 |
| 先刪文件再補測試 | 反序會有無人把關的空窗期；S1 未完成不得進 S3 |
| 事先承諾瘦身後的行數 | 刪完才知道；估錯就照實回報，不靠刪真契約湊數字 |
| 把外部 repo 的細節「搬」到對方 repo | 本技能只負責刪除與留下指向連結；對方 repo 的內容是對方的工作 |
