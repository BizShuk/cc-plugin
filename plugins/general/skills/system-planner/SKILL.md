...
> `僅規劃 (Only Planning)`：本技能只產出架構演進計畫，不實作、不重構、不修改任何
> 程式碼或設定，唯一寫入的檔案是 `./plans/` 下的計畫報告。
...
`<name>` 取目標路徑最後一段。報告寫入
`./plans/architecture-evolution-<name>.md`（目錄不存在先 `mkdir -p plans`）。
結構如下：

```markdown
# 架構演進與優化計畫 — <專案名稱> (Architecture Evolution & Optimization Plan)
